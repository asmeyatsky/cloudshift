"""Main web application integrating Azure Blob Storage, Cosmos DB, Service Bus, and Key Vault."""

from flask import Flask, request, jsonify
from blob_service import upload_blob, download_blob, list_blobs, generate_sas_url
...
@app.route("/blobs/<name>/sas", methods=["GET"])
def api_blob_sas(name):
    url = generate_sas_url(name, expiry_hours=2)
    return jsonify({"sas_url": url})
...
def api_search_items():
    ...
    results = query_items("SELECT * FROM c WHERE c.category = @cat", [{"name": "@cat", "value": category}])
